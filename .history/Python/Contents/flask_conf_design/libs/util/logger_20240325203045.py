...
# sys.argv --> 파이썬 스크립트에 전달된 명령행 파라미터를 전달받기 위해 사용하는 모듈
...
def get_caller(step=1, fullname=True):
    ...
    """
    step += 1
    caller = sys._getframe(step).f_locals.get('self') # _getframe : 현재 depth 의 함수 명을 가져옴
...
# '%H:%M:%S %y%m%d' --> 인시던트 발생용 sf log 용 formatter
sms_formatter = "%(levelname)s %(message)s %(asctime)s"


def create_sms_logger(log_file, logger_name="sms", stream_enabled=True, format=sms_formatter):
    """
